        ...
        try:
            ...
            with connection.cursor() as cursor:
                ...
                if result and result[0] == 1:
                    ...
                    self.stdout.write(
                        self.style.ERROR(
                            "Failed to connect to the database: Unexpected result from query."
                        )
                    )
            ...
            self.stderr.write(
                "1.  Verify your DATABASE_URL in settings.py and .env (if used)."
            )
            ...
            if "SSL" in str(e):
                self.stderr.write(
                    "5.  Ensure SSL/TLS is configured correctly (if required)."
                )
            ...
            self.stderr.write(
                "2. Check the SQL syntax (though the query is very simple)."
            )
