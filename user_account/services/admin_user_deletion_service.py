    ...
    def _delete_oauth_data(user):
        ...
        try:
            ...
            if SocialAccount.objects.filter(user=user).exists():
                ...
                # Log the deletion
                logger.info(
                    f"Deleted {len(social_accounts)} social accounts for user {user.id}"
                )
    ...
    def delete_user_and_related_data(user):
        ...
        try:
            with transaction.atomic():
                ...
                total_deleted = (
                    sum(item["count"] for item in deleted_objects) + 1
                )  # +1 for user
                user_id = user.id
                user_email = user.email
                user_username = user.username
            ...
            with transaction.atomic():
                ...
                for user in unverified_users:
                    try:
                        ...
                        logger.error(
                            f"Error deleting unverified user {user.id}: {str(e)}"
                        )
                        continue
