...
class NeonKeepAliveMiddleware:
    def __init__(self, get_response):
        ...
        if getattr(settings, "NEON_KEEPALIVE_ENABLED", True):
            ...
            self.ping_thread = threading.Thread(
                target=self._keep_alive, daemon=True, name="neon_keepalive"
            )
            self.ping_thread.start()
            logger.info("Background connection monitor started")
            print(
                f"{Fore.GREEN}✓ Background connection monitor active{Style.RESET_ALL}"
            )
    ...
    def _run_initial_connection_check(self):
        try:
            print(
                f"{Fore.YELLOW}⚡ Attempting initial database connection...{Style.RESET_ALL}"
            )
    ...
    def _keep_alive(self):
        ...
        while True:
            try:
                with connection.cursor() as cursor:
                    cursor.execute(
                        "SELECT 1, current_database(), now() AT TIME ZONE 'UTC'"
                    )
                    res = cursor.fetchone()

                    # Console feedback
                    print(
                        f"{Fore.GREEN}✓ {res[2]}: Connection active{Style.RESET_ALL}",
                        end="\r",
                    )
