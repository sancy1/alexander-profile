...
class CustomSocialAccountAdapter(DefaultSocialAccountAdapter):
    def populate_user(self, request, sociallogin, data):
        ...
        if (
            not user.role
        ):  # Check if role is already set (it shouldn't be, but it's safer)
            user.role = (
                UserRole.USER.value
            )  # Make *ABSOLUTELY* sure this is the correct value.
    ...
    def _get_valid_email(self, sociallogin):
        ...
        email = user_email(sociallogin.user) or sociallogin.account.extra_data.get(
            "email"
        )
    ...
    def _handle_existing_user(self, request, sociallogin, existing_user):
        ...
        if existing_social.exists():
            logger.info(
                f"Connecting to existing {provider} account for {existing_user.email}"
            )
        ...
        if existing_user.socialaccount_set.exists():
            logger.warning(
                f"Email {existing_user.email} exists with different social provider"
            )
            raise ImmediateHttpResponse(
                JsonResponse(
                    {
