...
class NewsletterSubscriberAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Dates', {'fields': ('subscribed_at', 'unsubscribed_at'), 'classes': ('collapse',)}),
    )
...
class DeletedSubscriberAdmin(admin.ModelAdmin):
    ...
    date_hierarchy = 'unsubscribed_at'
    ordering = ('-unsubscribed_at',)
    actions = ['reactivate_selected_subscribers']

    fieldsets = (
        (None, {'fields': ('email',)}),
        ('Timestamps', {'fields': ('unsubscribed_at', 'deleted_at'), 'classes': ('collapse',)}),
    )

    def reactivate_selected_subscribers(self, request, queryset):
        """Action handler to reactivate selected addresses cleanly via the business service layer."""
        reactivated_count = 0
        failed_count = 0
        ...
        if failed_count > 0:
            self.message_user(
                request,
                f"Failed to reactivate {failed_count} subscriber(s). Check messages for details.",
                level=messages.WARNING
            )

    reactivate_selected_subscribers.short_description = "Reactivate selected deleted subscribers"