...
#     def create_contact(data, request=None):
#         """
#         Creates a contact form entry and saves it directly to the database.
#         Email dispatch logic removed to keep database transactions fast and clean.
#         """
#         print("\n=== Starting contact creation process ===")
#         serializer = ContactSerializer(data=data)
#         if serializer.is_valid():
#             contact = serializer.save()
#             logger.info(f"Contact submission successfully saved to DB. ID: {contact.id}")
#             return contact
...
#                     'website_url': getattr(settings, 'FRONTEND_URL', 'https://www.alexandercyril.xyz'), 
#                     'preferences_url': f"{getattr(settings, 'FRONTEND_URL', 'https://www.alexandercyril.xyz')}/preferences", 
#                     'view_in_browser_url': f"{ContactService.get_base_url()}/newsletter/view-in-browser/{subscriber.id}/", 
#                     'linkedin_url': 'https://www.linkedin.com/in/alexander-cyril-603a2528a/', 
#                     'github_url': 'https://github.com/alexcy1', 
...
#                         'current_month_year': timezone.now().strftime("%B %Y"),
#                         'website_url': getattr(settings, 'FRONTEND_URL', 'https://www.alexandercyril.xyz'),
#                         'feedback_url': f"{getattr(settings, 'FRONTEND_URL', 'https://www.alexandercyril.xyz')}/feedback/unsubscribe/", 
#                         'preferences_url': f"{getattr(settings, 'FRONTEND_URL', 'https://www.alexandercyril.xyz')}/preferences",
#                         'view_in_browser_url': f"{ContactService.get_base_url()}/newsletter/view-in-browser/confirmation/", 
#                         'linkedin_url': 'https://www.linkedin.com/in/yourlinkedinprofile', 
#                         'github_url': 'https://github.com/yourgithubprofile', 
#                         'twitter_url': 'https://twitter.com/yourtwitterprofile', 
#                     }
...
class NewsletterService:
    ...
    # contact/services.py (Updated subscription method)

    @staticmethod
    def subscribe_email(data):
        """
        Creates an active newsletter subscription entry directly in the database.
        All SMTP email dispatch functions have been stripped to ensure rapid, 
        fail-proof execution across all environments.
        """
        email = data.get('email')
        if not email:
            raise ValidationError({'email': 'This field is required.'})
        ...
        try:
            ...
            # Clean out historical unsubscription traces if they re-signup
            deleted_sub = DeletedSubscriber.objects.filter(email=email).first()
            if deleted_sub:
                deleted_sub.delete()
                logger.info(f"Removed {email} from deleted subscribers for re-subscription.")
            ...
            subscriber = NewsletterSubscriber.objects.create(email=email, is_active=True)
            logger.info(f"New active newsletter subscriber saved to DB: {email}")
    ...
    @staticmethod
    def unsubscribe_email(email):
        """Safely shifts active records to the soft-deleted historical tracking model log table."""
        if not email:
            return False, "Email is required"
        ...
        try:
            ...
            if settings.EMAIL_HOST_USER:
                try:
                    ...
                    logger.error(f"Failed to send unsubscribe email trace to {email}: {str(e)}")
    ...
    def reactivate_deleted_subscriber(email):
        if not email:
            return {'success': False, 'message': 'Email is required for reactivation.'}, 400

        try:
            subscriber = NewsletterService.subscribe_email({'email': email})
            return {'success': True, 'message': f'Subscriber {email} reactivated successfully.', 'subscriber_id': subscriber.id}, 200
        except ValidationError as e:
            # FIXED: Extracted detail cleanly to prevent breaking parsing interfaces
            error_msg = e.detail.get('email', e.detail) if isinstance(e.detail, dict) else e.detail
